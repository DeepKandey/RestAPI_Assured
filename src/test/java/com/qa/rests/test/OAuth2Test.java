...
public class OAuth2Test {
	...
	public void OAuth2DemoUsingCOOP() {
		...
		// 2. POST the request
		Response responseForAccessToken = requestSpecificationForAccessToken.request(Method.POST,
				"http://coop.apps.symfonycasts.com/token");

		// 3. Status Code
		System.out.println(
				"Response Code for request to generate access token:--> " + responseForAccessToken.getStatusCode());
		...
		RequestSpecification oAuth2RequestSpecification = authenticationSpecification.oauth2(accessToken);

		// 4. POST the request
		Response oAuth2Response = oAuth2RequestSpecification.request(Method.POST,
				"http://coop.apps.symfonycasts.com/api/609/chickens-feed");

		// 5. Status Code
		System.out.println("Response Code for the post request using OAuth2:--> " + oAuth2Response.getStatusCode());
